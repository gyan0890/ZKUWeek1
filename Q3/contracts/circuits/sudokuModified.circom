...
template RangeProof(n) {
    ...
    //Even if one of them is 0, the final output will be zero.
...
template sudoku() {
    ...
    component rangeProofPuz[9][9];
    component rangeProofSol[9][9];


    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeProofPuz[i][j] = RangeProof(4);
            rangeProofSol[i][j] = RangeProof(4);

            rangeProofPuz[i][j].range[0] <== 0;
            rangeProofPuz[i][j].range[1] <== 9;

            rangeProofSol[i][j].range[0] <== 0;
            rangeProofSol[i][j].range[1] <== 9;
        }
    }
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rangeProofPuz[i][j].in <-- puzzle[i][j];
            assert(rangeProofPuz[i][j].out == 1);

            rangeProofSol[i][j].in <-- solution[i][j];
            assert(rangeProofSol[i][j].out == 1);
